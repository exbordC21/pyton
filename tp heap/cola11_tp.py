...
def luke_han(lacola):
    ...
    han=[]
    for i in range (lacola.size()):
        if lacola.on_front()[-2]=="Luke Skywalker":#el de luk no aria falta hacerlo ya que en la primera consigna
            luke.append(lacola.on_front()[-1])     #se muestra de que planeta es pero lo hice igual solo para 
            lacola.move_to_end()                   #cumplir con cada punto 
        elif lacola.on_front()[-2]=="Han Solo":
            han.append(lacola.on_front()[-1])
            lacola.move_to_end()
        else:
            lacola.move_to_end()
    print("Luke Skuwalker se encuentra en el planeta",luke)
    print("Han solo se encuentra en el planeta",han)
luke_han(lacola)
