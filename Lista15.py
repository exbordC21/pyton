...
# (tipo y subtipo);
print()
def tipos_fuego(list_completa):
    for index,elements in enumerate(list_completa):
        pos0=search(list_completa,'nombre',list_nombre[index])
        if pos0 is not None:
            pos = search(list_completa[pos0]['pokemones'],'tipo','Fuego')
            if pos is not None:
                pos2=search(list_completa[pos0]['pokemones'],'subtipo','Planta')
                if pos2 is not None:
                    print("los entrenadores que tienen pokemones de tipo fuego y planta son", list_completa[pos0]['nombre'])
    return()
tipos_fuego(list_completa)

def tipos_agua(list_completa):
    for index,elements in enumerate(list_completa):
        pos0=search(list_completa,'nombre',list_nombre[index])
        if pos0 is not None:
            pos = search(list_completa[pos0]['pokemones'],'tipo','Agua')
            if pos is not None:
                pos2=search(list_completa[pos0]['pokemones'],'subtipo','Volador')
                if pos2 is not None:
                    print("los entrenadores que tienen pokemones de tipo agua y volador son", list_completa[pos0]['nombre'])
    return()
...
def promedio_nivel(list_completa):
    ...
    return print("el promedio basado en el nivel de los pokemones de",list_completa[pos]['nombre'],"es de:",promedio,"%")
...
# i. mostrar los entrenadores que tienen Pokémons repetidos;
print()
def pokemon_repe(list_completa):
...
def entx_pokemony(list_completa,list_nombre,list_pokemon):
    ...
    for index, elements in enumerate(list_completa):
        ...
        if pos is not None:
            ...
            return print("el entrenador no existe, no sea realizara nada")
entx_pokemony(list_completa,list_nombre,list_pokemones) 
